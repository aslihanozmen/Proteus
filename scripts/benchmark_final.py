...
classpath = '/home/olib92/apps/xformer/xformer-union.jar:/home/olib92/apps/xformer/lib/*'
...
def runXformer_functional(line):
    ...
    try:
        output = subprocess.check_output(
            ['java', '-Xms8g', '-Xmx128g', '-cp', classpath, main, join(mypath, filename), input_array[1], input_array[2], 'UF', seed], timeout=1800).decode('utf-8')
        print(output)
        list = output.split('\n')
        ...
        try:
            queries3 = queries[0].split(':')[1]
            queries5 = queries[1].split(':')[1]
            queries10 = queries[2].split(':')[1]
        except IndexError:
            queries3 = queries5 = queries10 = -999
        ...
        line3 = [filename, seed, 3, precision3, recall3, queries3, iterations3, not_found3]
    ...
    with open('benchmark/results_' + xformer_name + '.csv', 'w', newline='') as outputFile:
        ...
        for file in file_content:
            ...
            try:
                ...
                print(message)
                lines.append([filename, '', '3', '','','','',file_len(join(mypath, filename)), file_len(join(mypath, filename)), time.time()-start_time])
